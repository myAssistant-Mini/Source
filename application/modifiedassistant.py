...
class Mini:
    ...
    def run_mini(self):
        try:
            ...
            task = self.hearYou()
            taskDictionary = {'intro': self.introduceSelf, 'contact': self.getAContactNumber, 'time': self.tellTime,
            ...
            taskArray = ['intro', 'contact', 'time', 'date', 'selfie', 'novel', 'read', 'note',
                         'do', 'weather', 'instagram', 'email', 'creator', 'message', 'news']
            ...
            for variousTasks in taskArray:
                if variousTasks in task:
                    taskDictionary[variousTasks]()
